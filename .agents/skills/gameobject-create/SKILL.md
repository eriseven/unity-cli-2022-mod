...
When a Prefab Stage is open, use `create_prefab_stage_gameobject` instead. It creates a child of the Prefab contents root by default, so the object is saved by `save_prefab_stage` rather than becoming an override on the scene instance that opened the stage.
